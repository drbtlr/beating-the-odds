...
calc_bto <- function(.ranks) {
  ...
  .ranks %>%
    ...
    mutate(sig = case_when(
      (estimate > 0 & ci_high > 0) & (estimate > 0 & ci_low > 0) ~ "yes",
      (estimate < 0 & ci_high < 0) & (estimate < 0 & ci_low < 0) ~ "yes",
      TRUE ~ "no"
    ))
}
...
```{r execute, echo=TRUE}
# Execute function for each subject area
bto_math <- calc_bto(ranks_math) %>% 
  # Keep variables of interest. 
  # Note: we select and rename variables at the same time.
  select(first_hs_code = groupLevel, estimate_math = estimate,
         se_math = std.error, ER_math = ER, pctER_math = pctER, 
         ci_high_math = ci_high, ci_low_math = ci_low, bto_math = sig)
  
bto_read <- calc_bto(ranks_read) %>% 
  select(first_hs_code = groupLevel, estimate_read = estimate,
         se_read = std.error, ER_read = ER, pctER_read = pctER, 
         ci_high_read = ci_high, ci_low_read = ci_low, bto_read = sig)
```
