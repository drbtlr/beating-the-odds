...
This beathing-the-odds analysis uses a multilevel framework that incoroporates school-level information into the modeling process. To prepare the analytical dataset, we calculated school averages within each school year. These variables where then centered using the grand mean of each variable. 
...
Multilevel models are a powerful and flexible extension to conventional regression frameworks. This is one of the many reasons why they are so attractive to education researchers. However, this added flexibility can make fitting and interpretating such models a challenge. Here, we present a relatively simple multilevel model that takes into account school-level variation. We encourge you to read more on the topic of multilevel modeling and its application in beating-the-odds analyses. 
...
# Examine the coefficients and standard errors for each variable. Ask yourself 
# if the estimates are in the range of reasonable possiblity. If not, go back 
# and inspect your dataset and make sure there are no errors in data processing 
...
It's always helpful to plot the results of your analysis. We've found a nice scatter plot of beathing-the-odds schools can be an effective visualization for communicating the overall results of the analysis. 
...
**Purpose:** This analysis examines the distribution of beating-the-odds schools by performance level in math and reading. Results provide an overall summary of a beathing-the-odds analysis.  
...
# Example include, referecing academic literature, technical
...
df_bto %>% 
  ...
  theme(plot.title = element_text(size = 16, face = "bold"),
        ...
        legend.position = "top",
        legend.text = element_text(size = 11)) +
  labs(x = "Math Residual", y = "Number of Schools", fill = "",
       title = "Distribution of BTO Schools by Math Residual") 
