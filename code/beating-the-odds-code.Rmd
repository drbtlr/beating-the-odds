...
#### Create function to flag BTO schools and performance levels
...
calc_bto <- function(.ranks, .var) {
  ...
  .ranks %>%
    ...
    mutate(perform_lvl = case_when(
      # Positive
      between(pctER, 70, 80) ~ 1,
      between(pctER, 80, 90) ~ 2,
      pctER >= 90 ~ 3,
      # Negative
      between(pctER, 20, 30) ~ -3,
      between(pctER, 10, 20) ~ -2,
      pctER < 10 ~ -3,
      # Other
      TRUE ~ 0
    )) %>% 
    # Select and name variables of interest
    select(first_hs_code = groupLevel, 
           "estimate_{{ .var }}" := estimate, "pctER_{{ .var }}" := pctER, 
           "bto_{{ .var }}" := bto, "perform_lvl_{{ .var }}" := perform_lvl)
}
```
