...
tmp_ma <- left_join(bto_cuts, df_avg_prior_ach %>% mutate(first_hs_code = factor(first_hs_code))) %>% 
  select(perf = perf_math, avg_prank = avg_8_math_prank) %>% 
  mutate(subject = "Math")

tmp_rd <- left_join(bto_cuts, df_avg_prior_ach%>% mutate(first_hs_code = factor(first_hs_code))) %>% 
...
df_plot %>% 
  ...
  mutate(perf = fct_relevel(
    ...
    "Large\npositive\nincrease")) %>%
  filter(perf != "No\nsignificant\ndecrease") %>%
  ggplot(aes(factor(perf), avg_prank)) +
  #geom_boxplot(aes(fill=subject)) +
  geom_jitter(aes(color=subject), width = 0.3, alpha = 0.55)+
  ylim(0,100)+
