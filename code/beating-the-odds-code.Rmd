...
# Step 2: Create temp datasets for plotting
tmp_ma <- left_join(df_cuts, df_avg_prior_ach %>% mutate(first_hs_code = factor(first_hs_code))) %>% 
  select(perf = perf_math, avg_prank = avg_8_math_prank) %>% 
  mutate(subject = "Math")

tmp_rd <- left_join(df_cuts, df_avg_prior_ach%>% mutate(first_hs_code = factor(first_hs_code))) %>% 
