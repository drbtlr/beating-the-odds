...
df_8assess_avg <- faketucky %>%
  ...
  summarise(avg_8_read = mean(scale_score_8_read, na.rm = T),
            avg_8_math = mean(scale_score_8_math, na.rm = T)) %>%
  mutate(first_hs_code = as.numeric(first_hs_code),
...
#************************************************************ ADD - look at high status schools
#Look at schools with medium/large positive changes and the percentile rank of their cohort's 8th grade scores
high_math_stat <- df_cuts %>%
  filter(perf_math == 3 | perf_math == 2) %>%
  mutate(math_8gr_prank = ifelse(avg_8_math_prank < 50, "Below 50%",
                                 ifelse(avg_8_math_prank >= 50 & avg_8_math_prank <= 75, "50-75%", ">75%")))  %>%
  group_by(math_8gr_prank) %>%
  summarise(n = n())

high_read_stat <- df_cuts %>%
  filter(perf_read == 3 | perf_read == 2) %>%
  mutate(read_8gr_prank = ifelse(avg_8_read_prank < 50, "Below 50%",
                                 ifelse(avg_8_read_prank >= 50 & avg_8_read_prank <= 75, "50-75%", ">75%"))) %>%
  group_by(read_8gr_prank) %>%
  summarise(n = n())

#look at 8th grade performance and performance status
ggplot(df_cuts, aes(x = avg_8_math_prank, y = resid_math, group = factor(perf_math))) +
  geom_point(aes(color = factor(perf_math)))
...
high_math_avg <- df_cuts %>%
  mutate(med_large_math = ifelse(perf_math == 3 | perf_math == 2, "med/large pos status", "not med/large pos status")) %>%
  group_by(med_large_math) %>%
  summarise(avg_math = mean(avg_8_math, na.rm = T))

high_read_avg <- df_cuts %>%
  mutate(med_large_read = ifelse(perf_read == 3 | perf_read == 2, "med/large pos status", "not med/large pos status")) %>%
  group_by(med_large_read) %>%
  summarise(avg_read = mean(avg_8_read, na.rm = T))
#*************************************************************
